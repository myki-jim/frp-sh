...
- 服务端面板数据接口仅在设置 `FRPSH_ADMIN_TOKEN` 时开放，使用该独立令牌登录。不要与服务器共享密码使用相同值。浏览器 WebSocket 的令牌位于查询参数中，应使用 HTTPS，并避免在代理访问日志记录查询参数。
- 本地面板仅监听回环地址，检查 Host 和 Origin，且不会向网页发送服务器密码。分享命令中的 `SERVER_PASSWORD` 需手动替换；如房间启用了加密，还需补充相同的 `--key`。
