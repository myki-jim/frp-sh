...
- 打洞失败后优先走 **TURN 中继**（配置 `turn_providers`，UDP 中继，比私有 TCP 中继穿透性/延迟更好），TURN 也不可用才落到私有 TCP 中继
...
- **服务器设了密码**（`serve --password`）：私有 TCP 中继通道用密码派生密钥做
  ChaCha20-Poly1305 **流加密**，防第三方嗅探（运营商/中间人看不到内容；
  服务器持有密码，可解密转发）；TURN 中继用标准 long-term credential 认证
  （防冒用），但标准 TURN 的数据面本身是明文——需要机密性时同样配合 `--key`
- **`--key` 端到端加密**：P2P 直连流量用 `--key` 派生密钥加密，
  **服务器也无法解密**（即使密码泄露）

组合建议：自用/局域网直连多用 `--key`（端到端）；打洞失败走中继时，
服务器密码至少保证"路上不裸奔"。

### 信令服务器可以设置密码吗（防止别人乱建房间）？
...
之后客户端必须在配置中填写同一密码（`frp-sh config` 向导第 4 步，或直接改
`config.toml` 加 `password = "..."`）：

- **请求认证**：所有信令请求校验密码，错误/缺失 → 401 拒绝
- **中继认证 + 加密**：中继连接需带密码，流量加密
- 不设密码的服务器行为与原来完全一致（兼容旧客户端）
