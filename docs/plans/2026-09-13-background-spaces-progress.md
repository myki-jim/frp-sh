...
| 空间 CLI | `space create/list/members/invite/redeem/connect/revoke-invites/remove-member/leave/delete`；`space connect ID` 将本机虚拟网卡接入永久空间 |
...
1. 将 `space connect` 接入后台 profile/服务任务，并持久化本地空间选择；当前为安全的前台入口。
