...
状态：0.6.0 发布验收。永久空间、原生后台服务和自定义域名作为显式命令交付；自动后台空间恢复仍列在后续范围。
...
0.6.0 发布 `space` 与原生 agent 服务命令，并保留旧 `create`/`join` 和旧邀请的兼容逻辑。`space connect` 已建立独立虚拟 LAN；它仍是前台命令，未宣称自动加入后台 Profile。
